...
- `cd frontend; pnpm audit:prod` rejects moderate-or-higher production dependency advisories; `pnpm audit:dev` rejects high-or-critical findings across the full frontend toolchain.

## Architecture and Data Flow
- Public endpoint `GET /api/get_data` is defined by `HTTPServer.RegisterRoutes` in `router.go` and implemented by `handler.go::HTTPServer.GetData` → injected `classroomService.GetTodayClassrooms`. Operational endpoints are `/healthz` and `/readyz` (runtime status). Unknown non-API paths fall back to the embedded `index.html` (SPA fallback in `router.go`); unknown `/api` and `/api/*` paths return JSON 404 with the same request `LogID` header/body `log_id` as registered API routes. Gzip middleware uses Accept-Encoding q-value negotiation (`acceptsGzip`); health/readiness stay uncompressed.
...
- Refreshes are single-flight (`refresh_coordinator.go`): concurrent requests share one attempt; internal outcomes distinguish full success, partial success, and total failure. Total or partial outcomes set a 30-second backoff. Request-triggered workers keep initiator context values (including `log_id`) via `context.WithoutCancel` + `ClassroomRefreshLimit`, and do not inherit client cancellation. `warmup.go` owns the single context-cancellable scheduler; graceful shutdown cancels it before HTTP drain and then calls `WaitBackground`, which prevents new workers before waiting.
...
- JavaScript modules use ES modules, React hooks, and 2-space indentation. ESLint 9 flat config (`frontend/eslint.config.js`) enforces the existing recommended React/Hooks rules and `react-refresh/only-export-components` — keep component files exporting only components (hooks/constants go in separate `.js` files).
...
- Frontend auto-reload pauses while the document is hidden, uses rate-aware delays with jitter, and aborts hung `/api/get_data` fetches after 40s. `selectionReducer` is pure; preference persistence lives in `SelectionProvider`. Dark-mode pre-hydration uses `darkModeBootstrap.js` (no inline scripts under CSP `script-src 'self'`).
...
- PRs are validated by `.github/workflows/ci.yml`; direct pushes to `main` are validated by the `quality-gate` job in `release.yml` (frontend production/toolchain audits, lint/test/build, gofmt, vet, `go test -race`, govulncheck, `bash scripts/install_test.sh`, and shellcheck on `scripts/*.sh`).
...
- Toolchain: Go 1.25.12+ (or Go 1.26.5+), Node 22, pnpm 9.15.x. All GitHub Actions are pinned to 40-character commit SHAs; bump pins by resolving the new SHA with `git ls-remote` and updating both the ref and the comment.
...
- Installer downloads only from official GitHub release URLs by default. Never reintroduce automatic third-party proxy fallback. Operators may set an explicit `DOWNLOAD_BASE_URL` for a mirror they already trust; same-origin `checksums.txt` proves integrity, not independent publisher identity.
- Installer changes must preserve the transaction contract: download/checksum/extract/render before snapshot; root-only candidate and backup directories; same-filesystem temporary files plus atomic rename; snapshot of binary/env/systemd unit+enablement/Nginx site+enablement plus prior service active/enabled runtime state; post-commit daemon reload, Nginx validation, service restart/active check, Nginx reload, and loopback health check; automatic restoration that stops any newly started unit, reloads Nginx after site removal, and restores prior active/enabled state (never force-start a previously inactive service) before any success message. Keep `/api/` `proxy_read_timeout` at 60s (above the 30s refresh / 45s write budgets). Keep release asset names and the single-file `install.sh` layout unchanged.
