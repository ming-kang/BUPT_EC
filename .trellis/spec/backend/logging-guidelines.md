...
access only to that log directory. Installed operators can use `bupt-ec logs`
(as permitted by local journal ACLs); it must remain a fixed-unit `journalctl`
wrapper and never silently elevate. `bupt-ec config show` may display only the
fixed safe registry with password/token redaction, never raw env/source output.
If logging paths or systemd hardening change, update the owning
`scripts/installer/` fragment, regenerate `scripts/install.sh`, and update
